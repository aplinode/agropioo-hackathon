...
When SDD pays off, when it is overkill, how to keep a spec alive, and the honest limit of specs
...
"Build output" does not mean "black box"

This inversion has one failure mode: deciding that because code is an output, you never have to look at it again.

In July 2026, Thoughtworks engineer Valentina Servile published a field report from production systems she maintains with agents. Her finding is worth holding right next to this diagram. When an agent meets a tangled codebase, it makes more wrong assumptions, burns more context, and chases more dead ends. Bad design still costs money. The bill now arrives in tokens, retries, and unreliable output instead of only in human hours.

Her sharper point is the one that matters for this course: you cannot judge whether a responsibility sits in the right place by reading a spec. Coupling, duplication, and names that no longer match reality only become visible in the implementation.

So read the inversion precisely. The spec is the source of truth for **what the system does**. The code is still the only honest evidence of **how well the system is holding together**. See [Should we still design code for humans?](https://www.thoughtworks.com/insights/blog/programming-languages/should-still-design-code-humans).
...
The honest caveat on Spec-as-Source

Spec-as-Source carries the strongest claim in the table and the least field evidence behind it. In its maximalist form the claim is that code becomes an ephemeral artifact nobody needs to read.

Practitioners maintaining agent-written systems in production report the opposite pressure. Agent-generated code decays faster than human-written code, because agents rarely leave a file better than they found it, and they will find surprising ways to make a build go green. Because they work fast, the debt compounds earlier than human debt would.

So the level you claim does not reduce your obligation to look. It raises it. Every level above Spec-First needs a human periodically stepping back, reading what shipped, and reshaping it. Part 5 closes on exactly this.

For this course, **start at Spec-First and grow into Spec-Anchored.** Spec-as-Source is where the field is heading, but you earn it by mastering the first two. The *discipline* is identical at every level; only how strictly you keep the spec in sync changes.
...
A too-strict constitution poisons everything after it

A constitution sets the tone for everything after it: if it demands enterprise-grade testing, performance budgets, and heavy process for a weekend project, **every later phase inherits that weight** and a to-do app turns into a cathedral. Match the constitution to the stakes; you can raise the bar later.

You can watch this happen. Give a weekend habit-tracker a constitution that demands 90% test coverage, a performance budget, and a written decision record for every change, and the first feature ships with a benchmark harness, three layers of abstraction, and a decision log: a week of process for a button that adds a row to a list. Nobody asked for that; the constitution did, and every phase after it inherited the weight.
...
12 rules on test coverage %, performance budgets, commit-message format, and review steps, all for a weekend app
...
Skip that discipline and the tool choice becomes the requirement by accident. A spec says "store uploads in S3"; the plan and code follow; a month later a compliance deal needs storage on the company's own servers. The behaviour anyone actually cared about (uploads stay durable and retrievable) was never written down, only the vendor was, so switching means a refactor instead of a one-line change to the plan.

### 7\. Phase 3: Clarify by interview (make the AI ask *you*)[​](#7-clarify)

This is the highest-value, most-skipped step. Before building, **turn the questioning around**: instead of instructing the AI, have it interview *you* to reveal everything the spec left ambiguous. One prompt does most of the work:
...
-   A change you could describe in one sentence (a typo, one rule, one new field): just ask for it. Skip the plan. Forcing heavy process on a one-line fix is the overkill side of the same mistake.
...
The second check: does the shape still make sense?

Acceptance criteria prove the behaviour is right. They do not prove the system is still healthy. Those are two different questions, and only the first one is automated.

So every few build cycles, run a separate design pass. Ask the agent to list the files it touched, where a rule is now written in two places, and which names no longer describe what the thing does. Then read that part of the code yourself, because the agent is a poor judge of its own output here: it optimises for a passing build, not for a system that stays cheap to change after the hundredth feature.

This pass is also self-interested. Legible code costs fewer tokens to work in, and this may be the first moment in software history where you can price technical debt directly: look at the month's model bill.
...
Because all four artifacts are plain files, your spec is now under version control: you can diff it, review it in a pull request, and see exactly when behaviour was supposed to change. That is the jump from "a spec I wrote once" to "a spec that governs the repo."
...
The result is not just working code; it is working code **plus a spec that explains and governs it**, ready for the next person (or the next you) to change safely.
...
Running *"make this button blue"* through the full constitution-to-implement process is absurd. But the moment a task involves state, permissions, data models, money, or anyone else's expectations, the structure starts paying for itself, and it pays more the longer the thing lives.
...
**Keep the spec alive (the part everyone forgets).** A spec is only the source of truth if it stays true. When behaviour changes (a new rule, a removed feature, a fixed edge case), change the **spec first**, then re-derive the code. This is the move that turns Spec-First into Spec-Anchored, and it is the difference between a spec that grows more valuable over time and one that becomes a lie in your repo within a month.
...
**What changes when you have many specs.** One spec is a document; a folder of dozens is a system. With a whole `specs/` directory, the live question stops being "is this spec clear?" and becomes "do these specs still agree?" Features interact, so a change to one can ripple into others, and when two specs point to conflicting choices the constitution is what settles it. Keeping the whole set of specs consistent as it grows is the real jump from "I can spec a feature" to "I run a repo on SDD."

### The limit of specs: what the code still owes you[​](#the-limit-of-specs)

Everything above argues for writing the spec first. It is worth closing on the strongest argument against carrying that idea too far, for two reasons: you will meet this argument, and taking it seriously makes you better at SDD rather than worse.

The maximalist version of spec-driven development goes like this. Specs become the real programming language. Code becomes an ephemeral artifact. No human ever reads an implementation again, and agents handle generation, review, deployment, and the 2am incident. Thoughtworks engineer Valentina Servile argued against that view in July 2026, from experience running production systems with agents. Three of her points are ones this course accepts outright.

**First: a spec precise enough to pin down design starts to look like code.** Natural language is accessible, and that is its whole advantage. It is also ambiguous. Once your spec fixes module boundaries, error handling, naming rules, testing strategy, and every constraint that matters, you have written a program in a language with no compiler and no type checker. You have not escaped programming. You are doing it with fewer guarantees. Dijkstra made this argument in 1978, and it has not aged: formal notation is not a burden we tolerate, it is the thing that stops us from writing confident nonsense.

**Second: agents are not deterministic, so a human sits on the far side of the black box.** A compiler given the same input returns the same output, every time. An agent asked for the same feature five times returns five designs, each with different trade-offs baked in. Someone has to decide whether the trade-offs it happened to choose are the right ones for this system. That decision is judgment, and it does not live in a spec file.

**Third: agent-written code decays, and the agent will let it.** Most agent-generated codebases in the industry are still young. They have not yet met the hundredth feature or the second year in production. What we already know is that agents are worse than humans at leaving a file better than they found it, and that a spec written once and delegated forever has an older name in this industry: Waterfall.

**How SDD survives all three.** The answers are already in this course. They are worth stating as answers.
...
Acceptance criteria become **executable checks**, not prose. The formal, unambiguous part of your spec is the test suite (Concept 8)

Agents are non-deterministic

You review the **plan** before any code, and the **result** after every step. Those gates exist for this reason (Concept 8)

Code decays quietly

The spec is your fixed point for a design pass, not a replacement for one. Read what shipped (the second check, Concept 8)

Spec once, then delegate forever

That is Waterfall, not SDD. "Keep the spec alive" is the whole difference (Concept 13)

The reframing worth keeping: **the audience for good design has expanded, it has not disappeared.** Your code now has two readers. A human who must judge whether the system still makes sense, and an agent that must change it next week. Both readers suffer from the same defects: tangled dependencies, one rule written in three places, names that lie about what they do. Both readers get faster and cheaper when the design is clean.

So the discipline is not "spec instead of code." It is: agree on the what, generate the how, then go look at what came out.

One tangled codebase, two billsBad designcoupling · duplication · lying namesReader 1 — the humanslower review, weaker judgmentintent misread, then re-arguedbugs found late, at full priceReader 2 — the agentmore context burned per changemore wrong assumptions mademore tokens, retries, rabbit holesSame defects, two invoices. Design for both readers, not neither.

*Two readers, one bill: the defects that slow a human reviewer are the same ones that make an agent burn context and guess wrong.*
...
**Project 3: keep the spec alive (the hard one, ~1 hr).** Now change your mind. Add a new requirement to Project 2: say, *tags can be colour-coded*, or *filtering supports "any of" and "all of" modes.* Resist the urge to just tell the agent to add it. Instead: **edit `spec.md` first**, re-run Clarify on the changed section, update the plan and tasks, then implement. **Done when** the diff on `spec.md` and the diff on the code tell the same story. Then add one pass most people skip: read the code that came out of both rounds and name one place where the shape is now worse than it was, a rule written twice, a module doing two jobs, a name that no longer fits. Fix that too, and note it in the spec if the spec caused it. This is the move that turns Spec-First into Spec-Anchored, and it is the one most people never practise.

**Project 4: work inside code you didn't write (Claude Code or OpenCode, ~2 hrs).** A brand-new project is the easy case. Clone any small open-source project you've never seen, or pick up someone else's repo, and add one modest feature using SDD. This time **Phase 1 carries the weight**: before you spec anything, make the agent research how the existing code is structured, where your feature has to fit, and what conventions it must respect; use subagents so each maps one area and reports back. Your spec must include a "fits the existing system" section that names the patterns you're matching. **Done when** your feature reads like it was always part of the codebase, not bolted on, and your spec explains *why* it fits.
...
### What is the core difference between vibe coding and spec-driven development?
