/* GET    /api/app-control/conversations/[id] — fetch a single conversation
   PATCH  /api/app-control/conversations/[id] — rename a conversation
   DELETE /api/app-control/conversations/[id] — delete a conversation and its messages */

import { query, queryOne } from "@/lib/db";
import { requireSessionApi } from "@/lib/auth/guards";
import { errorResponse, jsonResponse, readJsonBody } from "@/lib/http";
import { renameConversationSchema } from "@/lib/validation/app-control";
...
export async function GET(
  ...
  const conv = await queryOne<{ id: string; title: string; language: string; created_at: string; updated_at: string }>(
    `SELECT id, title, language, created_at, updated_at
     FROM app_control_conversations
...
export async function DELETE(
  ...
  await query(
    `DELETE FROM app_control_conversations WHERE id = $1 AND account_id = $2`,
    [id, session.accountId]
  );
