...
- **FR-4.9:** The diagnosis is returned in the farmer's selected language. The Hugging Face model is prompted to respond in the user's locale language. If the model cannot honor the language, the response falls back to English.
...
- **FR-8.4:** The model returns a structured prediction with disease name, confidence score, and severity. The server maps the model's 38 PlantVillage classes to human-readable Pakistani crop names and treatment advice.
