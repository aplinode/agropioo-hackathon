...
**Tests**: Not explicitly requested in the feature specification. Test tasks are omitted. Manual acceptance run-through per constitution is assumed.
...
- [ ] T009 Create the weighted multi-criteria scoring function with 5 dimensions (suitability, weather, profitability, risk, sustainability), normalisation, and configurable weights in `lib/crops/scoring.ts`
...
- [ ] T012 Create the `recommendCrops()` orchestration function that wires together validation, uniqueness check, weather fetch, market data fetch, soil lookup, catalogue filter, scoring, reason generation, and persistence in `lib/crops/engine.ts`
...
- [ ] T013 [US1] Implement `POST /api/crops` route handler — validate input with Zod, call `recommendCrops()`, return 201/409/503 per contract — in `app/api/crops/route.ts`
- [ ] T014 [P] [US1] Implement `GET /api/crops` route handler — list recommendation requests for a farm with cursor pagination per contract — in `app/api/crops/route.ts`
...
- [ ] T019 [US1] Create the i18n bundle helper `getCropsBundle()` for server-side translation fetching in `app/(farmer)/(dashboard)/crops/crops-bundle.ts`
- [ ] T020 [US1] Create the client-side recommendation form (farm selector, soil type dropdown, irrigation selector, budget bracket selector, submit) using `react-hook-form` + `@hookform/resolvers` with the Zod schemas from T002 — in `app/(farmer)/(dashboard)/crops/crops-client.tsx`
...
- [ ] T025 [US2] Create the revenue comparison bar chart component using the project's approved charting library (verify mandi feature's choice; if none approved, propose one for approval) — in `app/(farmer)/(dashboard)/crops/comparison-chart.tsx`
...
- [ ] T028 [P] [US3] Implement `GET /api/crops/save` route handler — fetch saved farm plan entry with rotation suggestions for a (farm, season, year) per contract — in `app/api/crops/save/route.ts`
...
- **Phase 2**: T007 and T008 can run in parallel (different files, no dependencies)
- **Phase 3**: T014, T015, T016, T017 can run in parallel once T013 is complete (different route files, same lib dependency)
- **Phase 5**: T028 can run in parallel with T027 (same file but different handler, no blocking)
