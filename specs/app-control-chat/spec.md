...
The app-control chat extends the existing advisor agent system. It reuses the same triage agent and multi-agent routing infrastructure, but with a dedicated set of app-control tools (create_record, update_record, delete_record, navigate_to_page, set_reminder, schedule_action, etc.). The agent architecture is shared; only the tool sets and routing rules differ between advisor and app-control modes. The agent has access to summaries of all previous conversations, allowing it to maintain context across sessions.
