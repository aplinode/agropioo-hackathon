...
export function createGetDashboardSummaryTool(accountId: string) {
  const raw = tool({
    name: "get_dashboard_summary",
    description:
      "Get a quick summary of the farmer's dashboard: farm count, total and recent record counts, upcoming season tasks, and any alerts. Use this when the farmer asks 'what's on my dashboard', 'show my overview', 'how many farms do I have', 'how many records', or 'what do I need to do today'.",
    parameters: z.object({}),
    async execute() {
      ...
      const totalRecords = await query<{ count: string }>(
        `SELECT count(*)::text AS count FROM records WHERE account_id = $1`,
        [accountId]
      );
...
${recordsCount === 0 ? "No recent activity — consider logging irrigation, spraying, or harvesting." : ""}`;
  ...
  const schema = raw.parameters as { required?: string[] };
