...
export function createUpdateRecordTool(accountId: string) {
  return tool({
    name: "update_record",
    description:
    ...
    async execute({ recordId, farmId, type, eventDate, title, note, yieldQty, laborCost, transportCost, confirmed }) {
      const existing = await queryOne<{ id: string; farm_id: string; type: string; event_date: string; title: string | null; note: string | null; yield_qty: string | null; labor_cost: string | null; transport_cost: string | null }>(
        `SELECT id, farm_id, type, event_date, title, note, yield_qty, labor_cost, transport_cost FROM records WHERE id = $1 AND account_id = $2`,
        [recordId, accountId]
      );
      ...
      if (farmId) {
        ...
        if (farmOwner.length === 0) {
          return "Target farm not found or does not belong to you.";
        }
      }
      ...
      await query(`UPDATE records SET ${sets.join(", ")} WHERE id = $1 AND account_id = $${idx}`, [...vals, accountId]);
