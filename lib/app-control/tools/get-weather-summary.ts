...
export function createGetWeatherSummaryTool(accountId: string) {
  return tool({
    ...
    async execute({ farmId }) {
      if (farmId) {
        const farm = await queryOne<{ lat: number; lng: number; name: string; location: string }>(
          `SELECT lat, lng, name, location FROM farms WHERE id = $1 AND account_id = $2 AND archived_at IS NULL`,
          [farmId, accountId]
        );
        if (!farm) {
